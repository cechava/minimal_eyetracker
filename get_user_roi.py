...
def main():
    ...
    # source_dir = os.path.join(options.rootdir, options.optionsanimalid, options.sessionn)
    # raw_folder = [r for r in os.listdir(run_dir) if 'raw' in r and os.path.isdir(os.path.join(source_dir, r))][0]
    ...
    #get image folder
    img_folder = os.path.join(options.source_dir,'frames')
    ...
    #option to define seed bounding box for pupul
    while True:
    ...
    if 'pupil' in roi_rect.keys():
        ...
        #fine tune pixel value threshold for pupil
        while True:
    ...
    #option to define seed bounding box for corneal reflection
    while True:
        cv2.imshow("source image", display_image)
        key = cv2.waitKey(1) & 0xFF
    ...
    #fine tune pixel value threshold for corneal reflections
    ...
    #save restriction box info to file
    output_file = os.path.join(options.source_dir,'user_restriction_box.json')
